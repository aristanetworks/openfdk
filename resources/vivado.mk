...
$(BUILD_DIR)/%/project.runs/impl_1/board_top.bit: BITFILE_DEPS=$(shell jq -r '. | \
	{constrs_1,sources_1}[][] | \
	sub("\\$${ARISTA_FDK_DIR}";"$(realpath $(ARISTA_FDK_DIR))") | \
	sub("\\$${PROJECT_DIR}";"$(realpath $(PROJECT_DIR))") | \
	sub("\\$${BUILD_DIR}";"$(realpath $(BUILD_DIR))")' \
...
$(BUILD_DIR)/%/project.xpr: \
			...
			$$(shell jq -r '. | {constrs_1,sources_1}[][] | \
	sub("\\$$$${ARISTA_FDK_DIR}"\\\;"$(realpath $(ARISTA_FDK_DIR))") | \
	sub("\\$$$${PROJECT_DIR}"\\\;"$(realpath $(PROJECT_DIR))") | \
	sub("\\$$$${BUILD_DIR}"\\\;"$(abspath $(BUILD_DIR))")' $(PROJECT_DIR)/src/$$*-cfg.json )
...
targets::
	@printf "%s\n" \
		...
		'                   Xilinx Vivado 2019.2 - Full License' \
		'' \
		'    <bitstream>:' \
