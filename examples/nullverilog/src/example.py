...
#
#  Description:
#    MOSAPI integration for the NullVerilog example design.
#
    ...
    def get_fpga_idx(self, label):
        for fpga in self.fpgas:
            if label in fpga.label:
                return "Fpga{}".format(self.fpgas.index(fpga) + 1)
        return None
...
def show_null_status(ctx):
    """show nullverilog status - show nullverilog status
    Group: Application nullverilog
    Mode: priv
    """
...
def show_registers(ctx):
    ...
    Group: Application nullverilog
    Mode: config-app-nullverilog
    """
    app = ctx.mode_ctx["app"]
    if not app.is_shutdown():
        print("Default Registers -")
...
def read_register(ctx, fpga="default", name=""):
    ...
    Usage: read register [default] NAME
    Group: Application nullverilog
    Mode: config-app-nullverilog
    """
    app = ctx.mode_ctx["app"]
...
def write_register(ctx, fpga="default", name="", value=""):
    ...
    Usage: write register [default] NAME HEXNUMBER
    Group: Application nullverilog
    Mode: config-app-nullverilog
    """
    app = ctx.mode_ctx["app"]
